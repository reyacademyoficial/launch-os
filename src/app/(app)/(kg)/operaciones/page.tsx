...
export default async function OperacionesDashboardPage({
  ...
  function buildHref(nextRange: RangeFilter): string {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <ContextBar
        ...
        title="Operaciones"
        stats={[
          { l: "Proyectos abiertos", v: fCount(activeProjectsCount) },
          { l: "Tareas abiertas", v: fCount(openTasksCount) },
          {
            l: "Vencidas",
      ...
      >
        ...
        <KpiCard
          ...
          hint={`${people.length} personas activas`}
        />
        <KpiCard
          label="Bloqueadores abiertos"
      ...
      >
        <Panel title="Productividad por persona">
          {productivityRows.length === 0 ? (
            <EmptyState
              ...
              hint="Cargá personas en Organización → Personas para ver métricas por persona."
            />
          ) : (
            <table
              style={{
        ...
        <Panel title="Bloqueadores críticos">
          {oldBlockers.length === 0 ? (
            <EmptyState

...
import { deleteProject } from "@/app/(admin)/admin/proyectos/actions";

export const metadata: Metadata = { title: "Proyectos · Configuración" };
...
export default async function ConfigProyectosPage() {
  await requireRole("superadmin");

  const supabase = await createClient();
  const { data } = await supabase
    .from("projects")
    .select("id, name, business_name, ownership, created_at")
          ...
          <table className="w-full min-w-[640px] text-sm">
            <thead className="bg-surface text-left text-xs uppercase tracking-wide text-fg-subtle">
              <tr>
                ...
                <th scope="col" className="px-4 py-3 font-medium">Razón social</th>
              ...
              {projects.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-xs text-fg-muted capitalize">
                      {p.ownership}
                    </td>

...
export const metadata: Metadata = {
  title: "Distribución de ingresos · Configuración",
};
...
/**
 * Distribución de ingresos — antes conocido como "Reglas de split".
 *
 * Define cómo se reparte cada cobro entre la organización y el proyecto:
 * un porcentaje sobre lo cobrado o vendido, más cargos fijos opcionales por
 * liquidación o venta. Cada proyecto puede tener una regla default + overrides
 * específicos por lanzamiento.
 *
 * Las páginas de creación/edición viven en /organizacion/reglas-split/new y
 * /organizacion/reglas-split/[id]/edit — se abre en el mismo shell.
 */
export default async function ConfigDistribucionPage() {
    ...
    <section className="space-y-6">
      <header>
        ...
        <p className="mt-2 max-w-3xl text-xs text-fg-muted">
          Define qué porcentaje de cada cobro recibe la organización por
          gestionar el proyecto. El motor resuelve la regla en dos pasos: busca
          primero un override específico por lanzamiento; si no existe, aplica
          la default del proyecto.
        </p>
      </header>
    ...
    <article className="rounded-md border border-border bg-surface p-5">
      <header className="mb-4 flex flex-wrap items-baseline justify-between gap-4">
        <div className="flex items-baseline gap-3">
          ...
          <span title="Ownership del proyecto">
      ...
      {!defaultRule && (
        <div className="mb-4 rounded-md border border-warning/40 bg-warning/10 p-3 text-xs text-warning-fg">
          ...
          y sus lanzamientos no se pueden liquidar hasta que exista una.
        </div>
      )}

      {defaultRule && (
        <RuleRow label="Default del proyecto" rule={defaultRule} />
      )}

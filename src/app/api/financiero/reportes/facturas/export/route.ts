...
import {
  buildInvoiceReportWorkbook,
  type InvoiceReportDetailExportRow,
} from "@/lib/finance/xlsx-export";
import { createClient } from "@/lib/supabase/server";

/**
 * GET /api/financiero/reportes/facturas/export
 *   ?status=&project=&range=|from=&to=
 *
 * Reporte de facturas en XLSX. Tres hojas: Resumen, Por estado, Detalle.
 * Filtros espejan la vista /financiero/reportes/facturas.
 */
...
export async function GET(request: Request) {
  ...
  // Nombres de proyecto para la columna del detalle.
  ...
  const detailExport: InvoiceReportDetailExportRow[] = filteredDetail.map((r) => ({
  ...
  const buffer = await buildInvoiceReportWorkbook(
    report.buckets,
    detailExport,
    periodLabel,
  );
  ...
  return new NextResponse(buffer as unknown as BodyInit, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="reporte-facturas-${stamp}.xlsx"`,

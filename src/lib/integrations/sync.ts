...
interface GhlProviderConfig {
   ...
   * `ghl_pipeline_lead_counts`. Opcional — sin pipeline configurada el sync
   * solo trae el count de leads nuevos por día.
   */
  pipeline_id?: string;
}
...
export async function syncLaunch(
  ...
  return await finalizeRun(service, runId, "success", upsertedDays, {
    insights_days: upsertedDays,
  });
}
...
async function runGhlBranch(args: {
  ...
  if (summary.status === "success") {
    // `rows_written` = días de `launch_daily_ads` escritos con el count de
    // leads. El conteo de pipeline se sobreescribe entero cada corrida, así
    // que no aporta un delta significativo para este número.
    return await finalizeRun(
      service,
      runId,
      "success",
      summary.counts.daily_rows_written,
      {
        cause: "ghl_summary",
